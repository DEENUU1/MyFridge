...
from dishes.models import Dish, TimeToMake, Country, DishCategory, DifficultyLevel, Type, MainIngredient, OtherIngredient, CustomUser
...
def test_create_rate_view(client, user, dish):
    ...
    response = client.post(reverse('social:rate-add', kwargs={'pk': dish.pk}), {
        'choose_rate': 5,
        'comment': 'Test comment'
    })

    assert response.status_code == 302 
    assert Rate.objects.filter(choose_rate=5, comment='Test comment', dish=dish, author=user).exists()


@pytest.mark.django_db
def test_update_rate_view(client, user, dish):

    rate = Rate.objects.create(choose_rate=3, comment='Initial comment', dish=dish, author=user)

    client.login(username='testuser', password='testpassword')

    response = client.post(reverse('social:rate-update', kwargs={'pk': rate.pk}), {
        'choose_rate': 4,
...
def test_delete_rate_view(client, user, dish):
    rate = Rate.objects.create(choose_rate=3, comment='Test comment', dish=dish, author=user)
...
def test_delete_from_favourites_view(client, user, dish):
    ...
    response = client.post(reverse('social:favourite-remove', kwargs={'favourite_id': favourite_dish.pk}))
